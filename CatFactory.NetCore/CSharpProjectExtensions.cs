...
namespace CatFactory.NetCore
{
    public static class CSharpProjectExtensions
    {
        ...
        public static string GetPropertyName<TProjectSettings>(this CSharpProject<TProjectSettings> project, ITable table, Column column) where TProjectSettings : class, IProjectSettings, new()
            => table.Name == column.Name ? string.Format("{0}1", project.GetPropertyName(column.Name)) : project.GetPropertyName(column.Name);

        public static string GetPropertyName<TProjectSettings>(this CSharpProject<TProjectSettings> project, IView view, Column column) where TProjectSettings : class, IProjectSettings, new()
