...
public class CSharpClassBuilder : CSharpCodeBuilder
{
    ...
    protected virtual void AddProperties(int start)
    {
        ...
        {
            var prop = ClassDefinition.Properties[i];

            if (prop.Attributes.Count > 0)
                this.AddAttributes(prop, start);

            if (prop.IsReadOnly)
            {
                if (prop.GetBody.Count == 0)
                {
                    Lines.Add(new CodeLine("{0}{1} {2} {3} {{ get; }}", Indent(start + 1), prop.AccessModifier.ToString().ToLower(), prop.Type, prop.Name));
                }
                else
                {
                    ...
                    if (prop.IsOverride)
                        propSignature.Add("override");
                    else if (prop.IsVirtual)
                        propSignature.Add("virtual");

                    propSignature.Add(prop.Type);

                    propSignature.Add(prop.Name);

                    Lines.Add(new CodeLine("{0}{1}", Indent(start + 1), string.Join(" ", propSignature)));

                    if (prop.GetBody.Count == 1)
                    {
                        Lines.Add(new CodeLine("{0}=> {1}", Indent(start + 2), prop.GetBody[0].Content.Replace("return ", string.Empty)));
                    }
                    else
                    {
                        Lines.Add(new CodeLine("{0}{1}", Indent(start + 1), "{"));
                        ...
                        foreach (var line in prop.GetBody)
                        {
                            if (line.IsComment())
                                Lines.Add(new CommentLine("{0}{1}", Indent(start + 2 + line.Indent), GetComment(line.Content)));
                            else if (line.IsReturn())
            ...
            else if (prop.IsAutomatic)
            {
                ...
                propertySignature.Add(prop.Type);

                propertySignature.Add(prop.Name);

                if (string.IsNullOrEmpty(prop.InitializationValue))
                    Lines.Add(new CodeLine("{0}{1} {{ get; set; }}", Indent(start + 1), string.Join(" ", propertySignature)));
                else
                    Lines.Add(new CodeLine("{0}{1} {{ get; set; }} = {2};", Indent(start + 1), string.Join(" ", propertySignature), prop.InitializationValue));
            }
            else
            {
                Lines.Add(new CodeLine("{0}{1} {2} {3}", Indent(start + 1), prop.AccessModifier.ToString().ToLower(), prop.Type, prop.Name));
                ...
                foreach (var line in prop.SetBody)
                {
                    if (line.IsComment())
                        Lines.Add(new CommentLine("{0}{1}", Indent(start + 3 + line.Indent), GetComment(line.Content)));
                    else if (line.IsPreprocessorDirective())
